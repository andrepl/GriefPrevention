...
public class BlockListener implements Listener {
    ...
    // constructor
    public BlockListener(GriefPreventionTNG plugin) {
        this.plugin = plugin;
    }
        ...
        if (xchange != 0 || zchange != 0) {
            for (Block block : blocks) {
                ...
                if (newOwnerName != null && !newOwnerName.equals(originalOwnerName)) {
                    plugin.getLogger().info("Removing piston because block (" + block + ") would change owner from " + originalOwnerName + "(claim:" + originalClaim.getId() + ") to " + newOwnerName + " (claim:" + newClaim.getId() + ")");
                    event.setCancelled(true);
                    event.getBlock().getWorld().createExplosion(event.getBlock().getLocation(), 0);
                    event.getBlock().getWorld().dropItem(event.getBlock().getLocation(), new ItemStack(event.getBlock().getType()));
                    event.getBlock().setType(Material.AIR);
