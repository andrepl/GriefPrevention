...
package com.norcode.bukkit.griefprevention.visualization;
...
// represents a visualization sent to a player// FEATURE: to show players visually where claim boundaries are, we send them fake block change packets// the result is that those players see new blocks, but the world hasn't been changed.  other players can't see the new blocks, either.
public class Visualization {
    public ArrayList<VisualizationElement> elements = new ArrayList<VisualizationElement>();

   // sends a visualization to a player
    public static void apply(GriefPreventionTNG plugin, Player player, Visualization visualization) {
    ...
    public static void Revert(GriefPreventionTNG plugin, Player player) {
        PlayerData playerData = plugin.getDataStore().getPlayerData(player.getName());
