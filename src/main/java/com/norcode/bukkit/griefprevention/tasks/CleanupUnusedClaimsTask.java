...
public class CleanupUnusedClaimsTask implements Runnable {
    ...
    public CleanupUnusedClaimsTask(GriefPreventionTNG plugin) {
        this.plugin = plugin;
    }
    ...
    public void run() {
        ...
        // if configured to always remove claims after some inactivity period without exceptions...
            ...
            if (needsInvestmentScan || creativerules) {
                ...
                if (removeClaim) {
                    plugin.getDataStore().deleteClaim(claim, null, true);
                    plugin.getLogger().info("Removed " + claim.getFriendlyOwnerName() + "'s unused claim @ " + GriefPreventionTNG.getfriendlyLocationString(claim.getMin()));
